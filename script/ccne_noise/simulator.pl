...
for(my $j=1;$j<=$times;$j++)
{
	...
	for(my $i= $start;$i<=$end;$i+=$step)
	{
		...
		my $rc = (rand(10) + 1)*100;
		system("~/bin/bbmap/randomreads.sh ref=DD01777.other.fasta out1=DD01777.other_$i.1.fq out2=DD01777.other_$i.2.fq seed=-1 length=150 coverage=$rc paired=t addpairnum=t");
		system("cat DD01777.chr_100.1.fq DD01777.KPC_$i.1.fq DD01777.other_$i.1.fq > ./$genome/DD01777.genome_$i.1.fq");
		system("cat DD01777.chr_100.2.fq DD01777.KPC_$i.2.fq DD01777.other_$i.2.fq > ./$genome/DD01777.genome_$i.2.fq");
	}
	chdir("$genome");
	system("pigz *");
	chdir("..");
